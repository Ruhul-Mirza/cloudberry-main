  ...
  if (
    ...
    result = result.filter((row) => {
      ...
      if (typeof rowValue === "object" && rowValue !== null) {
        return Object.values(rowValue).some((v) =>
      ...
      {columns.map((column, colIndex) => (
        <td key={`skeleton-col-${colIndex}`}>
          {column.type === 'avatar' ? (
            ...
            <Skeleton variant="text" width={column.width ? `${column.width * 0.7}px` : '70%'} />
          )}
        </td>
      ))}
    ...
    <>
      ...
      >
        ...
        >
          ...
          <tbody>
            {loading
              ? Array.from({ length: itemsPerPage }, (_, i) => renderSkeletonRow(i))
              : paginatedData.length === 0 ? (
                  <tr>
                    <td colSpan={columns.length + (showCheckbox ? 1 : 0) + ((onEdit || onDelete || onRename || onMove || actions.length > 0) ? 1 : 0)}>
                      <Box
                        sx={{
                          display: 'flex',
